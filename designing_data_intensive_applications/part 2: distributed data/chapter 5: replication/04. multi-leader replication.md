...
Some note-worthy strategies are:

- Conflict-free replicated datatypes (CRDTs): family of data structures for sets, maps, ordered lists and counters that can be edited by multiple users
  - they automatically resolve conflicts in a sensible way
- Mergeable persistent data structures: track history explicitly, similarly to a git version control system
  - use a 3-way merge function
  - CRDTs use a 2-way merge
- Operational transformation: it's the conflict resolution algorithm behind collaborative editing applications, such as Etherpad and Google Docs
  - designed for concurrent editing of an ordered list of items, such as a list of characters that constitute a text document
...
- circular and star topology:
  - cons:
    - the flow can be interrupted
  - pros:
    - message has a predictable path
- all-to-all:
  - cons:
    - some nodes maybe be faster, so some replication messages might overtake others (writes can arrive in the wrong order at some replicas)
  - pros:
    - increased fault tolerance, since the messages can travel through many different paths

> How do we deal with this issue on operations being processed out of order? Clocks (timestamp) can't be trusted with that. A technique used is **version vectors**
